...
class Lista {
	...
	public:
		...
		Lista();
		~Lista();
		Lista(const Lista &otra); // Construir uma lista a partir de otra en una direccia de memoria
		bool estaVacia();
		int tamano(); 
...
// Insertor un elemento en un indice especifico
template <typename TIPODATO>
...
cout << "Desplazando elemento " << items[i] <<  cout << i << " del indice";
cout << " al indice" << (i+1) << endl;
this->items[i+1] = this->items[i];
}

// Insertor nuevo elemento
